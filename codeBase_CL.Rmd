...
### Link PubMed search to dictionary
```{r, echo=TRUE, message=FALSE, warning=FALSE}
...
Sample records of unique **pmid** count on **meshHeadings**:
...
Histogram of top 40 disease or syndrome:

```{r, echo=FALSE, message=FALSE, warning=FALSE}
Top25 <- meSH_count[1:20,]

dt_filter$meshHeadings <- as.factor(dt_filter$meshHeadings)
meSH_popularity <- ggplot(Top25,aes(x=reorder(meshHeadings,Count,function(x)-length(x)),y=Count))
                        meSH_popularity + 
...
ggplot(data = dtPlot, aes(x=year, y=papers)) +
